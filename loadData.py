...
def loadDataAndEmbeddings(config,loadData):
    ...
    if loadData == True:
        ...
        wt = np.random.normal(0, 0.05, [len(source_word2idx), 300])  # (0, 0.05, [len(source_word2idx), 300])
        word_embed = {}
        count = 0.0
        ...
        train_size, train_polarity_vector = getStatsFromFile(FLAGS.train_path)  # train size 1880 in initial 2016 sample
        # print(train_size)
        # print(train_polarity_vector)
        # print("hey")
        test_size, test_polarity_vector = getStatsFromFile(FLAGS.test_path)
        # print(test_size)
        # print(test_polarity_vector)
