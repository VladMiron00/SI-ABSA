...
def main(train_path, test_path, accuracyOnt, test_size, remaining_size, learning_rate=0.09, keep_prob=0.3, momentum=0.85, l2=0.00001):
    ...
    with tf.Session(config=config) as sess:
        ...
        # tr_x = left context of train data (42 sen in small sample), max 80 token
        ...
        # te_x = left context of test data (25 sen in small sample), max 80 token
        ...
        for i in range(FLAGS.n_iter):
            ...
            for test, num in get_batch_data(te_x, te_sen_len, te_x_bw, te_sen_len_bw, te_y,
                ...
                ty = np.asarray(_ty) # actual labels
                py = np.asarray(_py) # predicted labels
                p = np.asarray(_p) # probability for each sentiment
                fw = np.asarray(_fw) # left context, 80 tokens
                bw = np.asarray(_bw) # right context, 80 tokens
                tl = np.asarray(_tl) # left target, 19 tokens
                tr = np.asarray(_tr) # right target, 19 tokens
                acc += _acc # numb of samples correctly predicted
                cost += _loss * num
                cnt += num # number of total samples
